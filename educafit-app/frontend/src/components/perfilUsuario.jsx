...
export default function PerfilUsuario({ usuario }) {
  ...
  useEffect(() => {
    ...
    fetch(`http://54.166.238.133/app-gimnasio/educafit-app/backend/obtenerDatosUsuario.php?id_usuario=${usuario.id}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setFormulario({
  ...
  const handleSubmit = e => {
    ...
    fetch("http://54.166.238.133/app-gimnasio/educafit-app/backend/actualizarDatosUsuario.php", {
      method: "POST",
